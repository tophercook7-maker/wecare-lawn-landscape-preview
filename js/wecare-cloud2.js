...
function applyRemote(store, rows){
  ...
  var merged;
  if(sessionValid()){
    // Authenticated owner: the pull sees everything, so trust it — a local row absent from
    // the cloud is a real delete (mirror). Pending (outbox) rows still win.
    var base=hasProt ? incoming.filter(function(o){ return o&&o.id&&!prot[o.id]; }) : incoming;
    merged=base.concat(keep);
  } else {
    // GUEST / not signed in: an anon pull returns RLS-hidden (usually empty) results.
    // NEVER treat that as deletions — keep EVERY local row; only apply incoming as
    // updates/additions. This is what was wiping just-made job cards on mobile when the
    // session briefly dropped to guest.
    var incById={}; incoming.forEach(function(o){ if(o&&o.id && !prot[o.id]) incById[o.id]=o; });
    merged=localRecsAll.map(function(o){ return (o&&o.id&&incById[o.id])? incById[o.id] : o; });
    var have={}; merged.forEach(function(o){ if(o&&o.id) have[o.id]=1; });
    incoming.forEach(function(o){ if(o&&o.id && !have[o.id]) merged.push(o); });
  }
  var cur;
